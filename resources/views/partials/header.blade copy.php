<header class="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
  <div class="container mx-auto flex h-14 items-center justify-between px-4 md:px-6">
    ...
    <nav class="hidden flex-1 items-center justify-center space-x-4 md:flex">
      @php
        $nav = [
  ...
  <aside data-mobile-sheet class="fixed inset-y-0 left-0 z-50 w-72 -translate-x-full transform bg-background shadow transition-transform duration-200">
    ...
    <nav class="flex flex-col space-y-1 p-4">
      @foreach($nav as $link)
        ...
        <a
          href="{{ $link['href'] }}"
          class="nav-link {{ $isActive ? 'active' : 'text-muted-foreground hover:text-foreground' }}"
          data-mobile-close
        >
          {{ $link['label'] }}
        </a>
