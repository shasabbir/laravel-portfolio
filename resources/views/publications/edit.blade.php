...
<div class="container mx-auto max-w-3xl px-4 py-10 md:px-6">
  ...
  <form method="POST" action="{{ route('publications.update', $publication) }}" class="space-y-4">
    ...
    <div>
      ...
      <input name="title" value="{{ old('title', $publication->title) }}" class="w-full rounded border border-input bg-background px-3 py-2" required />
    </div>
    <div>
      <label class="mb-1 block text-sm">Authors</label>
      <input name="authors" value="{{ old('authors', $publication->authors) }}" class="w-full rounded border border-input bg-background px-3 py-2" required />
    </div>
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
      <div>
        <label class="mb-1 block text-sm">Venue</label>
        <input name="venue" value="{{ old('venue', $publication->venue) }}" class="w-full rounded border border-input bg-background px-3 py-2" required />
      </div>
      <div>
        <label class="mb-1 block text-sm">Year</label>
        <input name="year" value="{{ old('year', $publication->year) }}" class="w-full rounded border border-input bg-background px-3 py-2" required />
      </div>
      <div>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
      <div>
        ...
        <input name="doi" value="{{ old('doi', $publication->doi) }}" class="w-full rounded border border-input bg-background px-3 py-2" />
      </div>
      <div>
        <label class="mb-1 block text-sm">URL</label>
        <input name="url" value="{{ old('url', $publication->url) }}" class="w-full rounded border border-input bg-background px-3 py-2" />
      </div>
      <div>
        <label class="mb-1 block text-sm">PDF</label>
        <input name="pdf" value="{{ old('pdf', $publication->pdf) }}" class="w-full rounded border border-input bg-background px-3 py-2" />
      </div>
    </div>
    <div>
